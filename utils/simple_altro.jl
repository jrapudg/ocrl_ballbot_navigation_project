...
function backward_pass!(params,X,U,P,p,d,K,reg,μ,μx,ρ,λ)
    ...
    for k = (N-1):(-1):1
        ...
        # dynamics jacobians
        A = FD.jacobian(_x -> discrete_dynamics(model,_x,U[k],dt),X[k])
        B = FD.jacobian(_u -> discrete_dynamics(model,X[k],_u,dt),U[k])
...
function forward_pass!(params,X,U,K,d,ΔJ,Xn,Un,μ,μx,ρ,λ; max_linesearch_iters = 20)
    ...
    for i = 1:max_linesearch_iters
        ...
        for k = 1:(N-1)
            ...
            Xn[k+1] = discrete_dynamics(model,Xn[k],Un[k],dt)
        end
...
function iLQR(params,X,U,P,p,K,d,Xn,Un;atol=1e-3,max_iters = 250,verbose = true,ρ=1,ϕ=10)
    ...
    dt = params.dt
    for i = 1:N-1
        X[i+1] = discrete_dynamics(model,X[i],U[i],dt)
    end
